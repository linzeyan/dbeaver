...
impl Renderer for DuckDb {
    ...
    /// written rather than about what DuckDB can do.
    fn column_change(
        &self,
        ...
        Err(DbError::new(
            "changing a column has not been written for DuckDB yet",
        ))
    }
    ...
    fn index_change(&self, _relation: &RelationInfo, _change: IndexChange<'_>) -> DbResult<String> {
        Err(DbError::new(
            "making or removing an index has not been written for DuckDB yet",
        ))
    }
    ...
    fn constraint_change(
        ...
        Err(DbError::new(
            "adding or removing a constraint has not been written for DuckDB yet",
        ))
    }
