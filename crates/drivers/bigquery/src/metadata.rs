    ...
    pub async fn columns(
        ...
        Ok(table
            ...
            .map(|(at, field)| ColumnInfo {
                ...
                // BigQuery states a generated column's expression in a different
                // field from the default's, so the two never arrive confused with
                // each other — which is the confusion this flag exists to undo.
                // Reading that field is one more thing no server has confirmed,
                // and a generated column is a table property nobody has asked
                // this driver for yet.
                computed: None,
            })
            .collect())
