...
impl Numeric {
        ...
        R: SqlReadBytes + Unpin,
    {
        ...
        Self::decode_body(src, len, scale).await
    }

    /// The sign and the digits, for a caller that already knows how many bytes
    /// of them there are.
    ///
    /// Split out from `decode` for `sql_variant`, which states the length of the
    /// whole value in its own header and so does not repeat it in front of the
    /// digits the way a `decimal` column does.
    pub(crate) async fn decode_body<R>(
        src: &mut R,
        len: u8,
        scale: u8,
