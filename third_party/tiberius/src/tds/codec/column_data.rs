        ...
        R: SqlReadBytes + Unpin,
    {
        let res = match ctx {
            ...
            TypeInfo::VarLenSizedPrecision { ty, scale, .. } => match ty {
                ...
                // Only decimals are described with a precision, so nothing
                // should arrive here. It reports rather than panics because a
                // decoder that aborts the process is the defect this whole
                // patch exists to remove.
                _ => {
                    return Err(Error::Protocol(
                        format!("{:?} does not carry a precision and a scale", ty).into(),
                    ))
                }
            },
