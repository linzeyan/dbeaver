...
impl Column {
    ...
    ///
    /// Without a CLR type name, because one is only ever read off the wire: a
    /// column constructed by hand is not a UDT column.
    ...
    /// The name of the CLR type for a [`ColumnType::Udt`] column: `geography`,
    /// `geometry`, `hierarchyid`, or whatever somebody registered.
    ///
    /// `None` for every other column type. This is the only thing distinguishing
    /// one UDT from another — they share a type byte and their values are opaque
    /// byte strings — so it is what a caller has to decode the value with.
